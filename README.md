...
output_matureRNA_type1.txt - Type 1 matches at RNA level
output_matureRNA_type2.txt - Type 2 matches at RNA level
Output Columns
Gene-level output:
...
Sequence Processing
Gene sequences: Extracted based on GTF gene coordinates
RNA sequences: Constructed by concatenating exons from transcripts
Reverse strand handling: Automatically reverse-complemented for scanning
Upstream sequence: 20bp immediately upstream of match start (genomic coordinates)
Use Cases
...
Limitations
Performance: Whole-genome scanning can be time-consuming for large genomes
Memory Usage: Loading large FASTA files requires sufficient RAM
GTF Compatibility: Requires standard GTF format with gene and exon features
Pattern Complexity: Complex patterns with many degenerate bases may reduce specificity
Troubleshooting
Common Issues
"Error: Pattern format is invalid"

Ensure pattern follows LEFT(MIDDLE)RIGHT format
Check for special characters or spaces
"KeyError" when fetching sequences
...
Debug Mode
Add debug prints in the code to:
